...
def single_step(input_arr,mode):
    ...
    #判断input_arr长度，等于2直接合成，大于2循环合成
    #select_output为形成的SQL语句中所有列名的array
    format_str = select_str
    select_output = []
    if len(input_arr)==2:
        if mode ==1:
            format_str += blank_str + count_str +left_str +input_arr[0] +right_str + blank_str + as_str + blank_str+count_num+blank_str+from_str +blank_str+input_arr[-1]
        if mode==2:
        ...
        for it in input_arr[:-2]:
            format_str += blank_str + it+comma_str
        if mode==1:
            format_str += blank_str + count_str +left_str+input_arr[-2]+right_str + blank_str +as_str+blank_str+ count_num
        elif mode==2:
            format_str += blank_str + avg_str + left_str + input_arr[-2] + right_str+ blank_str+as_str+blank_str +avg_num
        elif mode==3:
            format_str += blank_str + sum_str + left_str + input_arr[-2] + right_str + blank_str+as_str+blank_str +sum_num
        format_str += blank_str +from_str + blank_str+input_arr[-1] +blank_str + group_str
...
if __name__ == '__main__':
    ...
    out_put_3,single_join = join_step(single_num,query_4,1,"userId","userId",single_output,input_col4)
